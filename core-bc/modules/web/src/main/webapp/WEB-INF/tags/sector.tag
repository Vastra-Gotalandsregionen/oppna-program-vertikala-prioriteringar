...
<div class="sector-node">
  <c:choose> 
    ...
    <c:otherwise>
      <input disabled="disabled" style="border:none; background-repeat:no-repeat; background-image:url('img/node-open-disabled.png'); width:15px; height:15px;" id="Sektor${sector.id}" type="submit" src="img/unchecked.png" name="openId" value="            ${sector.id}" />
    </c:otherwise>
  </c:choose>
