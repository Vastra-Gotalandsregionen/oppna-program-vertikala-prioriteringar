...
<span class="kod-label ${key}-label" title="${prio.child != null ? su:mkChangedToolTip(prio.child[key], prio[key]) : ''}">
  ...
  <img src="img/flag_white.gif" title="${prio.child != null ? su:mkChangedToolTip(prio.child[key], null) : ''}" id="${key}ChangeFlag" style="display:${(prio.child != null and prio[key] != prio.child[key])? 'inline':'none'}"/>
</span> 
