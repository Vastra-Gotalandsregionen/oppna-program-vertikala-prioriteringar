  ...
  <div class="sector-node">
    ...
    <span>
      ...
      <c:if test="${item.deleteAble and not item.locked and not parentDeleted and item.prioCount == 0}">
        ...
        <c:if test="${item.markedAsDeleted}"> <img src="${pageContext.request.contextPath}/img/flag_red.gif" alt="Raderas vid sparande" /> </c:if>
      </c:if>
      <span>
      ...
      <c:if test="${item.prioCount > 0}">
        <span><img src="${pageContext.request.contextPath}/img/icon_alert.gif"  title="Kan ej raderas, har ${item.prioCount} prioriteringsobjekt." /> </span>
      </c:if>
      ...
      <c:choose>
        ...
        <c:when test="${item.locked and not item.markedAsDeleted}">
          <img src="${pageContext.request.contextPath}/img/icon_padlock.gif" alt="Låst för din användare" />
        </c:when>
      </c:choose>
